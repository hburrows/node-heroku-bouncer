  ...
  function reauthenticate(req, res) {
    ...
    if (req.method.toLowerCase() === 'get' && !isJSON) {
      req.session.redirectPath =
        // NOTE: the following logic assumes basePath ends with a slash
        req.url !== '/'
          ? options.basePath + (req.url[0] === '/' ? req.url.substring(1) : req.url)
          : options.basePath;
      console.log('***** node-heroku-bouncer redirecting to: ' + options.basePath + 'auth/heroku');
      res.redirect(options.basePath + 'auth/heroku');
    } else {
      res.statusCode = 401;
